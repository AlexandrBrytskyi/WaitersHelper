...
public class AdminService extends WaitersService implements IAdminService {
    ...
    public Ordering removeOrdering(Ordering ordering) {
        return orderingDAO.removeOrdering(server.persistentModel.order.Ordering.toPersistentOrdering(ordering))
                .toTransferOrdering();
    }
    ...
    public User addNewUser(User user) throws UserFieldIsEmptyException, WrongLoginException {
        return coocksMonitor.putNewMessageKey(userDAO.addNewUser(server.persistentModel.user.User.toPersistentUser(user))).toTransferUser();
    }

    public User removeUser(User user) {
        return userDAO.removeUser(server.persistentModel.user.User.toPersistentUser(user)).toTransferUser();
    }

    @Override
    public void lockUser(User user, boolean lock) {
        user.setLocked(lock);
        userDAO.mergeUser(server.persistentModel.user.User.toPersistentUser(user));
    }

    @Override
    public List<Denomination> getSoltDenominationsForReport(LocalDate concreteDate) {
        List<Denomination> founded = server.persistentModel.denomination.Denomination.getListTOdenoms(
                denominationDAO.getReadyDenominationsByDate(LocalDateTime.of(concreteDate, LocalTime.of(0, 0))));
    ...
    public List<Denomination> getSoltDenominationsForReport(LocalDate periodStart, LocalDate periodEnd) {
        List<Denomination> founded = server.persistentModel.denomination.Denomination.getListTOdenoms(
                denominationDAO.getReadyDenominationsByDate(LocalDateTime.of(periodStart, LocalTime.of(0, 0)), LocalDateTime.of(periodEnd, LocalTime.of(23, 59)))
        );
        return plusPortions(founded);
    }

    @Override
    public List<Denomination> getRequireDenominationsForReport(LocalDate concreteDate) throws DateException {
        if (concreteDate.compareTo(LocalDate.now())<0) throw new DateException("Wrong date!!!");
        List<Denomination> founded = server.persistentModel.denomination.Denomination.getListTOdenoms(
                denominationDAO.getDenominationsByDate(LocalDateTime.of(concreteDate, LocalTime.of(0, 0))));
        return plusPortions(founded);
    }

    @Override
    public List<Denomination> getRequireDenominationsForReport(LocalDate periodStart, LocalDate periodEnd) throws DateException {
        if (periodStart.compareTo(LocalDate.now())<0) throw new DateException("Wrong date!!!");
        List<Denomination> founded = server.persistentModel.denomination.Denomination.getListTOdenoms(
                denominationDAO.getDenominationsByDate(LocalDateTime.of(periodStart, LocalTime.of(0, 0)),
                        LocalDateTime.of(periodEnd, LocalTime.of(23, 59))));
        return plusPortions(founded);
