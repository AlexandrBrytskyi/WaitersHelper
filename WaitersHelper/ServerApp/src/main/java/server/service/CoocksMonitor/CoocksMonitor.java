...
//       else when list of logined users is updated and there is needed user denom will be added;
    ...
    protected void smartRemoving(Denomination denom) {
        ...
        if (loginOfCoocker == null || loginOfCoocker.equals("")) {
            ...
            System.out.println("login of coocker = " + loginOfCoocker + "\n " + tasksByWhoCoockingType.get(denom.getDish().getWhoCoockDishType()).get(userDAO.getUser(loginOfCoocker)));
    ...
    private class UserTask {
        ...
        public void removeTask(Denomination denomination) {
            removeFromNewTask(denomination);
            removeFromWorkingTask(denomination);
            portionsIDo -= denomination.getPortion();
            denominationDAO.removeCurrentDenomination(denomination.getId());
        }

        private void removeFromNewTask(Denomination denomination) {
           Denomination den = null;
            for (Denomination denomination1 : newTask) {
                if (denomination.getId()==denomination1.getId()) {
                    den = denomination1;
                    break;
                }
            }
        ...
        private void removeFromWorkingTask(Denomination denomination) {
            Denomination den = null;
            for (Denomination denomination1 : workingTask) {
                if (denomination.getId()==denomination1.getId()) {
                    den = denomination1;
                    break;
                }
            }
            workingTask.remove(den);
        }
