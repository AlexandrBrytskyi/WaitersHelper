...
public class OrderingDAO implements IOrderingDAO, Serializable {
    ...
    public Ordering setKO(double ko, Ordering ordering) {
        try {
            ...
            fund.setKo(ko);
            countPrice(sessionFactory.getCurrentSession().get(Ordering.class, ordering.getId()));
    ...
    private Fund countPrice(Ordering ordering) {
        ...
        for (Denomination denomination : sessionFactory.getCurrentSession().get(Ordering.class, ordering.getId()).getDenominations()) {
            if (!denomination.getState().equals(DenominationState.CANCELED_BY_ADMIN) &&
                    ...
                    (denomination.getState().equals(DenominationState.JUST_ADDED) && ordering.getType().equals(OrderType.PREVIOUS)))
